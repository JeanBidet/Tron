...
public class Paranoid implements Mindset{
    ...
    public Paranoid(Map map, int maxDepth, Heuristic heuristic, ArrayList<Champion> championsList, int id, Vector vector){
        this.map = map;
    ...
    private MoveHeuristic paranoid(int deep, int idChampion){
        ...
        for(Coord nextCoord : possibleCoords){
            ...
            // Détermine l'id du prochain joueur
            ...
            int[] heuristic = paranoid(deep+1, idNextChampion).getHeuristic();
            ...
            if(best.getHeuristic() == null){
                ...
                for(int i = 0; i < championsList.size(); i++){
                    if(championsList.get(i).getId() == idChampion){
                        ...
                        if(idChampion != this.id && paranoidSelection(best.getHeuristic(), heuristic, idChampion)){
                            best.setV(v);
                            best.setHeuristic(heuristic);
                        }
                        else if(best.getHeuristic()[i] < heuristic[i]){
                            best.setV(v);
                            best.setHeuristic(heuristic);
                        }
                    }
    ...
    private boolean paranoidSelection(int[] best, int[] heuristic, int id){
        int resHeuristic = 0;
        int resBest = 0;
        for(int i = 0; i < this.championsList.size(); i ++){
            if(i != id){
                resHeuristic += heuristic[i];
                resBest += best[i];
            }
        }
        return resHeuristic < resBest;
    }

    private int[] heuristicMultiPlayer(int idChampion){
        int[] heuristic = new int[championsList.size()];
    ...
    public void launch(Coord playerCoord, int deep, int alpha, int beta) {
        ...
        Vector newVector = paranoid(deep, this.id).getV();
